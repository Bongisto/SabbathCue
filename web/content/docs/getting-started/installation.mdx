---
title: Installation
description: Clone, install dependencies, and run the seven-phase data setup so SabbathCue is ready for development.
icon: Download
---
...
```bash
git clone https://github.com/BongaNdlovu/SabbathCue.git
cd SabbathCue
```
...
4. **ONNX model** — downloads `Qwen3/Qwen3-Embedding-0.6B`, exports
   it with the sentence-transformers ONNX backend, and quantizes it to
   INT8.
...
6. **Embedding precomputation** — runs the embeddings (auto-selects
   ...
   CPU).
7. **Local STT model** - prepares the local speech-to-text model files used by
   the offline Vosk provider.
...
bun run download:whisper           # Optional legacy Whisper model
```

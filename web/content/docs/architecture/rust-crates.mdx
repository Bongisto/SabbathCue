...
| `rhema-stt` | Deepgram STT WebSocket + REST fallback; Vosk local engine |
...
- **`rhema-stt`** owns network and local STT. Both Vosk and Deepgram
...
- `rhema-stt` exposes a `whisper` feature (`whisper-rs = "0.16"`,
  `optional = true`) for the legacy Whisper engine. The Vosk and
  Deepgram paths are always available.

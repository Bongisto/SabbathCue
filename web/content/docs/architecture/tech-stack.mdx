---
title: Tech Stack
description: Frontend, backend, ML, database, broadcast, and STT layers — the libraries SabbathCue is built on and the reasons we picked them.
icon: Layers
---
...
| **STT** | Local Vosk (open dictation), or Deepgram WebSocket + REST (`tokio-tungstenite`) |
...
- **Qwen3-Embedding-0.6B** for semantic search, exported to ONNX and
  quantized to INT8 with the sentence-transformers ONNX backend
...
- **Vosk** runs locally via a Python worker process in open dictation
  mode rather than a constrained phrase grammar.

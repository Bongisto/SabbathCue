...
description: Send SabbathCue's verse overlays into your live production switcher as a first-class NDI source — configurable resolution, frame rate, and alpha mode.
icon: Radio
---

import { Callout } from "fumadocs-ui/components/callout";

SabbathCue doesn't replace your live production tooling — it plugs into it.
...
1. **Licensing** — NDI's redistribution terms don't allow shipping the
   ...
   keeps SabbathCue clean of NDI binaries.
...
- **vMix** — Add → NDI / Desktop Capture → your SabbathCue source name.
...
SabbathCue's NDI sender (`crates/broadcast/src/ndi.rs`) emits BGRA frames
...
- **Audio**: not transmitted — SabbathCue is a graphics source only.
...
1. STT processing (Vosk runs locally; Deepgram streams within the WebSocket).
...
These numbers come from the dependencies, not from a SabbathCue benchmark

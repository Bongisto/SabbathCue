...
SabbathCue's detector doesn't bet on a single signal. Pastors cite scripture in
...
The detections panel shows each matched verse, the strategy that surfaced it
(direct, semantic, quotation, or reading mode), the confidence score, and the
transcript snippet that triggered the match.
...
**all-MiniLM-L6-v2** model (exported to INT8 ONNX during
...
Pastors quote across a sermon's arc. SabbathCue keeps a rolling **sermon
...
control API (`/sabbathcue/confidence`). Bumping it up reduces false positives
on noisy services; lowering it surfaces more candidates for manual
queueing.

## Golden replay fixtures

Detection changes are checked against labeled sermon replay fixtures in
`data/detection-fixtures/`. The closing-sermon golden fixture preserves the
operator transcript with timestamps and replays STT-sized windows through the
same direct + semantic + FTS hybrid path used by live detection:
...
The fixture distinguishes live-fire matches, held semantic hints, and silent
story/prayer windows. Hymn lyric matching is out of coverage: spoken hymnal
commands are routed separately, and sung lyric fragments are not treated as
hymn detections. If a lyric fragment clearly overlaps scripture, it may appear
as a held Bible hint, such as `Revelation 5:12` for "worthy is the lamb", but
it must not auto-fire.
